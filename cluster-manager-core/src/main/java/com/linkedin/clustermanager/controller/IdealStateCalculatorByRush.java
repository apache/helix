...
{
  /**
   * Build the config map for RUSH algorithm. The input of RUSH algorithm
   * groups nodes into "cluster"s, and different clusters can be assigned 
   * with different weights. 
   * @param numClusters number of node clusters 
   * @param instancesPerCluster List of clusters, each contain a list of node name strings.
   * @param replicationDegree the replication degree
   * @param clusterWeights the weight for each node cluster
   * @return this config map structure for RUSH algorithm.
   */
  static HashMap<String, Object> buildRushConfig(
      int numClusters,
      List<List<String>> instancesPerCluster,
   ...
   * @param numClusters number of node clusters 
   * @param instanceClusters List of clusters, each contain a list of node name strings.
   * @param instanceClusterWeights the weight for each instance cluster
  ...
  public static ZNRecord calculateIdealState(
      ...
      List<Integer> instanceClusterWeights, 
      int partitions,
      int replicas, 
      String dbName
      ) throws Exception
  {
    ...
    HashMap<String, Object> rushConfig = buildRushConfig(numberOfClusters, nodesInClusters,replicas + 1, clusterWeights);
    ...
    for (int i = 0; i < partitions; i++)
    {
        ...
        ArrayList<HashMap> partitionAssignmentResult = rushHash.findNode(partitionName);
  ...
  /**
   * Calculate the ideal state for list of instances. The function put all the instance in 
   * one single cluster.
      ...
      String dbName) throws Exception
  {
    ...
    HashMap<String, Object> rushConfig 
      = buildRushConfig(numberOfClusters, nodesInClusters,replicas, clusterWeights);
    RUSHrHash rushHash = new RUSHrHash(rushConfig);
   ...
   * the partition assignment ideal state.
   * */
