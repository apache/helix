 ...
 * */
public class RoutingInfoProvider
{
    static Logger _logger = Logger.getLogger(RoutingInfoProvider.class);
            ...
            List<ZNRecord> dbExternalViewList)
    {
        ...
        for (ZNRecord dbNodeView : dbExternalViewList)
        {
            ...
            for (String partitionId : dbNodeStateMap.keySet())
            {
                ...
                Map<String, String> nodeStateMap = dbNodeStateMap
                        .get(partitionId);
                for (String nodeName : nodeStateMap.keySet())
                {
                    ...
                    {
                        result.get(partitionId).put(state,
                                new TreeSet<String>());
                    }
            ...
            List<ZNRecord> idealStates)
    {
        ...
        if (idealStates != null)
        {
            for (ZNRecord idealState : idealStates)
            {
                ...
                resultRoutingTable.put(idealState.getId(),
                        defaultDBExternalView);
            }
        }
        else
        {
        ...
        for (String nodeName : currentStates.keySet())
        {
            ...
            for (ZNRecord dbNodeStateRecord : zndbStates)
            {
                ...
                {
                    Map<String, String> dbPartitionStates = dbStates
                            .get(stateUnitKey);
                    ...
                    if (!partitionStatus.getMapFields().containsKey(
                            stateUnitKey))
                    {
