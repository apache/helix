...
public class TransitionMessageGenerator {
	...
	public List<Message> computeMessagesForTransition(IdealState idealState,
		...
		for (String stateUnitKey : idealState.stateUnitSet()) {
			...
			for (String instanceName : instanceStateMap.keySet()) {
				...
				String currentState = _currentStateHolder.getState(
						stateUnitKey, instanceName);
				...
				if (desiredState == null
					...
					logger.warn("Cannot find ideal state for key:"
							+ stateUnitKey + " instance:" + instanceName);
					continue;
				}
				...
				if (!desiredState.equalsIgnoreCase(currentState)) {
					...
					String nextState;
					nextState = _stateModelDefinition
							.getNextStateForTransition(currentState,
									desiredState);
					if (nextState != null) {
						String pendingState = _messageHolder.get(stateUnitKey,
								instanceName);
						if (pendingState != null
								&& pendingState.equalsIgnoreCase(nextState)) {
							...
							logger.info("Adding message for " + instanceName
									+ "  " + stateUnitKey + " currentState="
									+ currentState + " nextState=" + nextState
									+ " desiredState=" + desiredState
									+ " pendingState=" + pendingState);
							Message message = createMessage(idealStateRecord,
									stateUnitKey, instanceName, currentState,
									nextState);
							messages.add(message);
						}
					} else {
						logger.warn("Unable to find a next state from stateModelDefinition"
								+ _stateModelDefinition.getClass()
								+ " from:"
	...
	private Message createMessage(ZNRecord idealStateRecord,
