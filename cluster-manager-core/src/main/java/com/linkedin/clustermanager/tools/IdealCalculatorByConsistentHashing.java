...
public class IdealCalculatorByConsistentHashing 
{
  ...
  /**
   * The default string hash function. Same as the default function 
   * used by Voldmort
   */
   ...
   * @param partitions the partition number of the database
   * @param replicas the replication degree
  ...
  {
	  return calculateIdealState(instanceNames,
	       partitions, 
	       replicas, 
	       dbName, 
	       hashFunc,
   ...
   * @param replicas the replication degree
   * @param dbName the name of the database
   * @param hashringSize the size of the hash ring used by consistent hashing
   * @return The ZNRecord that contains the ideal state
   */
  public static ZNRecord calculateIdealState(List<String> instanceNames,
          int partitions, 
          ...
          )
  {
    ...
    result.setId(dbName);
      ...
      // Put slaves in next has ring positions. We need to make sure that no more than 2 slaves
   ...
   * @param instanceNames List of instance names.
   * @param hashringSize the size of the hash ring used by consistent hashing
   * @return The int array as the hashing. it contains random values ranges 
   *         from 0..size of instanceNames-1
   */
  ...
  private static void putNodeValueOnHashRing(int[] hashRing, int nodeVal, int numberOfValues, List<Integer> positions)
  {
    ...
    for(int i = 0;i < numberOfValues; i++)
    {
      ...
      // swap the random index and the last available index, and decrease the nodes left 
   ...
   * @param result the hash ring array.
   * @param nodeValue the int value to be added to the hash ring this time
   * @param numberOfNodes number of node values to put on the hash ring array
  ...
  /**
   * Helper function to see how many partitions are mapped to different instances 
   * in two ideal states
   * */
  ...
  /**
   * Helper function to calculate and print the standard deviation of
   * the partition assignment ideal state, also the min/max of master partitions that 
   * is hosted on each node
   * */
...
/*    int steps = 10;
  ...
  static void printArrayStat(int[] vals)
  {
    ...
    System.out.println("normalied deviation: "+ Math.sqrt(deviation / vals.length) / mean);
  }
...
//  int[] ring1 = IdealCalculatorByConsistentHashing.generateEvenHashRing(instanceNames, 65535);
...
//    printArrayStat(ring1);
    ...
    ZNRecord result = IdealCalculatorByConsistentHashing.calculateIdealState(
       instanceNames, partitions, replicas, dbName, new IdealCalculatorByConsistentHashing.FnvHash());
    System.out.println("\nMaster :");
...
//    System.out.println(new String(bytes));
    ...
    ZNRecord result2 = IdealCalculatorByConsistentHashing.calculateIdealState(
       instanceNames2, partitions, replicas, dbName, new IdealCalculatorByConsistentHashing.FnvHash());
    ...
    int[] ring2 = IdealCalculatorByConsistentHashing.generateHashRing(instanceNames2, 30000);
    
    IdealCalculatorByConsistentHashing.compareHashrings(ring1, ring2);
    //printNodeStats(result);
    //printNodeStats(result2);
    bytes = serializer.serialize(result2);
    printHashRingStat(ring2);
//    System.out.println(new String(bytes));
 * */
