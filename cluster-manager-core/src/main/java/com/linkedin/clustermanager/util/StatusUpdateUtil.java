...
{ 
   ...
   * @param message the related cluster manager message
   * @param level the error level
   * @param classInfo class info about the class that reports the status update
   * @param additional info the additional debug information
   */
  public ZNRecord createMessageStatusUpdateRecord(
      Message message, 
      Level level, 
      Class classInfo,
  ...
  /**
   * Create a statusupdate that is related to a cluster manager message, then record it
   * to the zookeeper store.
   * @param message the related cluster manager message
   * @param level the error level
   * @param classInfo class info about the class that reports the status update
   * @param additional info the additional debug information
   * @param accessor the zookeeper data accessor that writes the status update to zookeeper
   */
  public void logMessageStatusUpdateRecord(
      Message message, 
      Level level, 
      Class classInfo,
      String additionalInfo,
  ...
  {
    logMessageStatusUpdateRecord(message, Level.ERROR, classInfo, additionalInfo, accessor);
  }
  ...
  {
    logMessageStatusUpdateRecord(message, Level.INFO, classInfo, additionalInfo, accessor);
  }
  ...
  {
    logMessageStatusUpdateRecord(message, Level.WARNING, classInfo, additionalInfo, accessor);
  }
  
  /**
   * Write a status update record to zookeeper
   * to the zookeeper store.
   * @param record the status update record
   * @param instanceName the instance name
   * @param accessor the zookeeper data accessor that writes the status update to zookeeper
   */
  public void publishStatusUpdateRecord(
      ZNRecord record, 
      String instanceName, 
      ClusterDataAccessor accessor
      )
  {
    accessor.setInstanceProperty(instanceName, InstancePropertyType.STATUSUPDATES, record.getId(), record);//, CreateMode.PERSISTENT_SEQUENTIAL);
  ...
  /**
   * Write an error record to zookeeper
   * to the zookeeper store.
   * @param record the status update record
   * @param instanceName the instance name
   * @param accessor the zookeeper data accessor that writes the status update to zookeeper
   */ 
  void publishErrorRecord(ZNRecord record, String instanceName,ClusterDataAccessor accessor)
  {
    assert(record.getSimpleField("LEVEL").equalsIgnoreCase("ERROR"));
    accessor.setInstanceProperty(instanceName, InstancePropertyType.ERRORS, record.getId(), record);//, CreateMode.PERSISTENT_SEQUENTIAL);
  }
}
