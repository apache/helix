...
public class ZKHelixAdmin implements HelixAdmin
{
  ...
  public void addInstance(String clusterName, InstanceConfig instanceConfig)
  {
    ...
    _zkClient
        .createPersistent(HelixUtil.getMessagePath(clusterName, nodeId), true);
    _zkClient.createPersistent(
        HelixUtil.getCurrentStateBasePath(clusterName, nodeId), true);
    _zkClient.createPersistent(HelixUtil.getErrorsPath(clusterName, nodeId), true);
    _zkClient.createPersistent(
        HelixUtil.getStatusUpdatesPath(clusterName, nodeId), true);
  }
  ...
  {
    return _zkClient.getChildren(HelixUtil
