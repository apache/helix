...
public class ZKHelixManager implements HelixManager
{
      ...
      InstanceType instanceType, String zkConnectString) throws Exception
  {
    ...
    try
    {
      sessionTimeoutInt = Integer.parseInt(System.getProperty(
          "zk.session.timeout", "" + DEFAULT_SESSION_TIMEOUT));
    } catch (NumberFormatException e)
    {
      logger.warn("Exception while parsing session timeout: "
          + System.getProperty("zk.session.timeout", ""
  ...
  private boolean isInstanceSetup()
  {
        ...
        || _instanceType == InstanceType.CONTROLLER_PARTICIPANT)
    {
      boolean isValid = _zkClient.exists(PropertyPathConfig.getPath(
          PropertyType.CONFIGS, _clusterName,
          ConfigScopeProperty.PARTICIPANT.toString(), _instanceName))
  ...
  public void addConfigChangeListener(ConfigChangeListener listener)
  {
    ...
    final String path = PropertyPathConfig.getPath(PropertyType.CONFIGS,
  ...
  public void addMessageListener(MessageListener listener, String instanceName)
  {
    ...
    checkConnected();
    final String path = _helixAccessor.keyBuilder().messages(instanceName)
        .getPath();
    CallbackHandler callbackHandler = createCallBackHandler(path, listener,
        new EventType[]
        { EventType.NodeChildrenChanged, EventType.NodeDeleted,
  ...
  void addControllerMessageListener(MessageListener listener)
  {
    ...
    checkConnected();
    final String path = _helixAccessor.keyBuilder().controllerMessages()
        .getPath();
  ...
  {
    logger.info("ClusterManager.addCurrentStateChangeListener() "
        + instanceName + " " + sessionId);
    checkConnected();
    final String path = _helixAccessor.keyBuilder()
        .currentStates(instanceName, sessionId).getPath();
      ...
      String instanceName)
  {
    ...
    checkConnected();
    final String path = _helixAccessor.keyBuilder().healthReports(instanceName)
        .getPath();
  ...
  private void addLiveInstance()
  {
    ...
    liveInstance
        .setLiveInstance(ManagementFactory.getRuntimeMXBean().getName());
    ...
    if (!_helixAccessor.setProperty(keyBuilder.liveInstance(_instanceName),
        liveInstance))
    {
  ...
  private void handleNewSessionAsParticipant()
  {
    ...
    // check info
    String healthCheckInfoPath = _helixAccessor.keyBuilder()
        .healthReports(_instanceName).getPath();
  ...
  public boolean isLeader()
  {
    ...
    Builder keyBuilder = _helixAccessor.keyBuilder();
    LiveInstance leader = _helixAccessor.getProperty(keyBuilder
        .controllerLeader());
    if (leader == null)
    {
      return false;
  ...
  private void carryOverPreviousCurrentState()
  {
    ...
    List<String> subPaths = _helixAccessor.getChildNames(keyBuilder
        .sessions(_instanceName));
    for (String previousSessionId : subPaths)
    {
      List<CurrentState> previousCurrentStates = _helixAccessor
          .getChildValues(keyBuilder.currentStates(_instanceName,
              previousSessionId));
      ...
      for (CurrentState previousCurrentState : previousCurrentStates)
      {
        if (!previousSessionId.equalsIgnoreCase(_sessionId))
        {
          ...
          StateModelDefinition stateModel = _helixAccessor
              .getProperty(keyBuilder.stateModelDef(stateModelDefRef));
          for (String partitionName : previousCurrentState
              .getPartitionStateMap().keySet())
          {

            previousCurrentState.setState(partitionName,
                stateModel.getInitialState());
          }
          previousCurrentState.setSessionId(_sessionId);
          _helixAccessor.setProperty(keyBuilder.currentState(_instanceName,
              _sessionId, previousCurrentState.getId()), previousCurrentState);
        }
      }
    }
    ...
    for (String previousSessionId : subPaths)
    {
      if (!previousSessionId.equalsIgnoreCase(_sessionId))
      {
        String path = _helixAccessor.keyBuilder()
            .currentStates(_instanceName, previousSessionId).getPath();
  ...
  public synchronized PropertyStore<ZNRecord> getPropertyStore()
  {
    ...
    if (_propertyStore == null)
    {
      ...
      _propertyStore = new ZKPropertyStore<ZNRecord>(zkClient,
