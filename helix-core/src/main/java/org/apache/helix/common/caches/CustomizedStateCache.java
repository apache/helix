...
public class CustomizedStateCache extends AbstractDataCache<CustomizedState> {
  ...
  // If the cache is already refreshed with customized state data.
  private boolean _initialized = false;
  private CustomizedStateSnapshot _snapshot;
  ...
  public CustomizedStateCache(ControlContextProvider contextProvider) {
    ...
    _snapshot = new CustomizedStateSnapshot(_customizedStateCache);
  }
  ...
  public boolean refresh(HelixDataAccessor accessor, Map<String, LiveInstance> liveInstanceMap,
    ...
    for (PropertyKey key : _customizedStateCache.keySet()) {
      ...
      if (customizedState != null && params.length >= 4) {
        ...
        String customizedStateName = params[2];
        ...
        Map<String, CustomizedState> customizedStateNameCustomizedStateMap =
            instanceCustomizedStateMap.get(customizedStateName);
        if (customizedStateNameCustomizedStateMap == null) {
          customizedStateNameCustomizedStateMap = Maps.newHashMap();
          instanceCustomizedStateMap.put(customizedStateName,
  ...
  private void refreshCustomizedStatesCache(HelixDataAccessor accessor,
    ...
    for (String instanceName : liveInstanceMap.keySet()) {
      ...
      if (_initialized) {
        _snapshot = new CustomizedStateSnapshot(newStateCache, _customizedStateCache, reloadedKeys);
      } else {
        _snapshot = new CustomizedStateSnapshot(newStateCache);
        _initialized = true;
      }
  ...
  /**
   * Provides the customized state of the node for a given customized state name
   * @param instance
  ...
  public Map<String, CustomizedState> getCustomizedState(String instance,
    ...
    if (!_customizedStateMap.containsKey(instance)
        || !_customizedStateMap.get(instance).containsKey(customizedStateName)) {
      return Collections.emptyMap();
    }
    return Collections.unmodifiableMap(_customizedStateMap.get(instance).get(customizedStateName));
  }

  @Override
  public CustomizedStateSnapshot getSnapshot() {
    return _snapshot;
  }
}
