...
public class PartitionWeightAwareEvennessConstraint extends AbstractRebalanceSoftConstraint {
  ...
  public PartitionWeightAwareEvennessConstraint(PartitionQuotaProvider partitionQuotaProvider,
  ...
  private int evaluate(String resource, String partition, String participant) {
    ...
    double usage = _capacityProvider.getParticipantProvisioned(participant) + (_pendingUsage
        .containsKey(participant) ? _pendingUsage.get(participant) : 0);
    double available =
        capacity - usage - _partitionQuotaProvider.getPartitionQuota(resource, partition);
  ...
  public void updateAssignment(ResourcesStateMap pendingAssignment) {
    Map<String, Integer> newParticipantUsage =
        ResourceUsageCalculator.getResourceUsage(pendingAssignment, _partitionQuotaProvider);
