...
public class TotalCapacityConstraint extends AbstractRebalanceHardConstraint {
  ...
  public TotalCapacityConstraint(PartitionQuotaProvider partitionQuotaProvider,
  ...
  private boolean validate(String resource, String partition, String participant) {
    int usage = _capacityProvider.getParticipantProvisioned(participant) + (_pendingUsage
        .containsKey(participant) ? _pendingUsage.get(participant) : 0);
    return +_partitionQuotaProvider.getPartitionQuota(resource, partition) + usage
