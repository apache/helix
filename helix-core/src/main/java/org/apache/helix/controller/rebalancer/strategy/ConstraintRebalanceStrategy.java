...
public class ConstraintRebalanceStrategy extends AbstractEvenDistributionRebalanceStrategy {
   ...
   * TODO Note this strategy will flip-flop almost for sure if directly used in the existing rebalancer.
   *      Enable different constraints for automatic rebalance process in the controller later.
   */
  public ConstraintRebalanceStrategy() {
    ...
    PartitionWeightAwareEvennessConstraint defaultConstraint =
        new PartitionWeightAwareEvennessConstraint(new PartitionQuotaProvider() {
          @Override
