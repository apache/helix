...
public class BestPossibleStateCalcStage extends AbstractBaseStage {
  ...
  private BestPossibleStateOutput compute(ClusterEvent event, Map<String, Resource> resourceMap,
    ...
    for (Resource resource : resourceMap.values()) {
      resourcePriorityQueue.add(new ResourcePriority(resource, cache.getIdealState(resource.getResourceName()),
          (HelixManager) event.getAttribute("helixmanager")));
    }

    Iterator<ResourcePriority> itr = resourcePriorityQueue.iterator();
    while (itr.hasNext()) {
      computeResourceBestPossibleState(event, cache, currentStateOutput, itr.next()._resource, output);
    }
  ...
  class ResourcePriority implements Comparable<ResourcePriority> {
    ...
    public ResourcePriority(Resource resource, IdealState idealState, HelixManager helixManager) {
      _resource = resource;

      if (helixManager != null && idealState != null
        ...
        JobContext jobContext = new TaskDriver(helixManager).getJobContext(resource.getResourceName());
