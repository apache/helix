...
public class TestStatusUpdate extends ZkStandAloneCMTestBase
{
  ...
  public void testParticipantStatusUpdates() throws Exception
  {
    ...
    for(ExternalView extView : extViews) {
      ...
      for(String key: partitionSet)
      {
        String partition = key.split("_")[1];
        Map<String, String> stateMap = extView.getStateMap(key);
        for(String instance : stateMap.keySet())
        {
          String state = stateMap.get(instance);
          StatusUpdateContents statusUpdates = StatusUpdateContents.getStatusUpdateContents(accessor, 
                                                                                            instance, 
                                                                                            resourceName, 
                                                                                            partition);
          Map<String, TaskStatus> taskMessages = statusUpdates.getTaskMessages();
          List<Transition> transitions = statusUpdates.getTransitions();
          if(state.equals("MASTER"))
          {
            ...
            Transition prevTransition = transitions.get(transitions.size()-2);
            Assert.assertEquals(taskMessages.get(lastTransition.getMsgID()), TaskStatus.COMPLETED, "Incomplete transition");
            Assert.assertEquals(taskMessages.get(prevTransition.getMsgID()), TaskStatus.COMPLETED, "Incomplete transition");
          ...
          else if(state.equals("SLAVE"))
          {
            ...
            Transition lastTransition = transitions.get(transitions.size()-1);
            Assert.assertEquals(lastTransition.getFromState().equals("MASTER") || lastTransition.getFromState().equals("OFFLINE"),
