...
public class TestWtCacheAsyncOpMultiThread extends ZkUnitTestBase
{
  ...
  class TestCreate implements Callable<Boolean>
  {
    final ZkCachedDataAccessor<ZNRecord> _accessor;
    final String                         _clusterName;
    final int _id;

    public TestCreate(ZkCachedDataAccessor<ZNRecord> accessor, String clusterName, int id)
    {
      _accessor = accessor;
    ...
    public Boolean call() throws Exception
    {
      ...
      for (int j = 0; j < 2; j++)
      {
        ...
        boolean[] success = _accessor.create(paths, records, Option.PERSISTENT);
  ...
  class TestUpdate implements Callable<Boolean>
  {
    final ZkCachedDataAccessor<ZNRecord> _accessor;
    final String                         _clusterName;
    final int _id;

    public TestUpdate(ZkCachedDataAccessor<ZNRecord> accessor, String clusterName, int id)
    {
      _accessor = accessor;
      _clusterName = clusterName;
      _id = id;
    ...
    public Boolean call() throws Exception
    {
      ...
      for (int j = 0; j < 10; j++)
      {
        ...
        boolean[] success = _accessor.update(paths, updaters, Option.PERSISTENT);
  ...
  class TestSet implements Callable<Boolean>
  {
    final ZkCachedDataAccessor<ZNRecord> _accessor;
    final String                         _clusterName;
    final int _id;

    public TestSet(ZkCachedDataAccessor<ZNRecord> accessor, String clusterName, int id)
    {
      _accessor = accessor;
      _clusterName = clusterName;
      _id = id;
    ...
    public Boolean call() throws Exception
    {
      ...
      for (int j = 0; j < 2; j++)
      {
        ...
        boolean[] success = _accessor.set(paths, records, Option.PERSISTENT);
  ...
  public void testHappyPath()
  {
    ...
    for (int i = 0; i < 2; i++)
    {
      threads.add(new TestCreate(accessor, clusterName, i));
    }
    ...
    for (int i = 0; i < 2; i++)
    {
      threads.add(new TestUpdate(accessor, clusterName, i));
    }
    TestHelper.startThreadsConcurrently(threads, 1000);
    ...
    for (int i = 0; i < 2; i++)
    {
      threads.add(new TestSet(accessor, clusterName, i));
    }
    TestHelper.startThreadsConcurrently(threads, 1000);
