  ...
  public static boolean afterClassCheck(String classname) {
    ...
    try {
       threadByName = threads.
          stream().
          filter(p -> p.getThreadGroup().getName() != null && p.getThreadGroup().getName() != "system").
          collect(Collectors.groupingBy(p -> p.getName()));
    } catch (Exception e) {
      System.out.println("filtering thread failure with exception:" + e);
    }
    ...
    for (ThreadCategory threadCategory : ThreadCategory.values()) {
      ...
      if (catThreadCnt != null) {
        ...
        if (catThreadCnt > limit) {
          checkStatus = false;
          System.out.println("Failure " + threadCategory.getDescription() + " has " + catThreadCnt + " thread");
          dumpThread = true;
        } else if (catThreadCnt > warningLimit) {
          System.out.println("Warning " + threadCategory.getDescription() + " has " + catThreadCnt + " thread");
          dumpThread = true;
        } else {
