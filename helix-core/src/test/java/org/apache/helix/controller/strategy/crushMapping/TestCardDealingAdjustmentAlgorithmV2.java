...
public class TestCardDealingAdjustmentAlgorithmV2 {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(TestCardDealingAdjustmentAlgorithmV2.class);
  ...
  public void testStableComputeMappingForMultipleTimes(int replica, int repeatTimes, int seed,
    ...
    while (repeatTimes > 0) {
      LOGGER.info("Round {} replica {} algorithm seed: {} preference: {}", repeatTimes, replica, seed, preference);
  ...
  public void testComputeMappingForDifferentPreference() {
    ...
    LOGGER.info(String.format("Total movements: %s, isAllAssigned: %s, preference: %s",
  ...
  public void testComputeMappingWhenZeroWeightInstance() {
    ...
    LOGGER.info("old mapping" + oldSimpleMapping);
    LOGGER.info("new mapping" + newSimpleMapping);
