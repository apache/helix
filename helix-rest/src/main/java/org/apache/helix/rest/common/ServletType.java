...
public enum ServletType {
  ...
  DEFAULT_SERVLET(HelixRestNamespace.DEFAULT_NAMESPACE_PATH_SPEC,
      new String[]{AbstractHelixResource.class.getPackage().getName(), NamespacesAccessor.class
          .getPackage().getName(), ZooKeeperAccessor.class
          .getPackage().getName(), MetadataStoreDirectoryAccessor.class
          .getPackage().getName(), LockAccessor.class.getPackage().getName()}),

  /**
   * Servlet serving namespaced API endpoints (/admin/v2/namespaces/{namespaceName})
   */
  COMMON_SERVLET("/namespaces/%s/*",
      new String[]{AbstractHelixResource.class.getPackage().getName(), ZooKeeperAccessor.class
          .getPackage().getName(), MetadataStoreDirectoryAccessor.class
          .getPackage().getName(), LockAccessor.class.getPackage().getName()});
