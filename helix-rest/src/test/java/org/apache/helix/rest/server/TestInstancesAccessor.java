...
public class TestInstancesAccessor extends AbstractTestClass {
  ...
  @Test
  public void testInstanceStoppableWithDisabledAndOfflineInstances() throws Exception {
    System.out.println("Start test :" + TestHelper.getTestMethodName());

    String instanceName = STOPPABLE_INSTANCES2.get(STOPPABLE_INSTANCES2.size() - 1);
    InstanceConfig instanceConfig = _configAccessor.getInstanceConfig(STOPPABLE_CLUSTER3, instanceName);
    instanceConfig.setInstanceEnabled(false);
    _configAccessor.setInstanceConfig(STOPPABLE_CLUSTER3, instanceName, instanceConfig);
    TestHelper.verify(
        () -> !_configAccessor.getInstanceConfig(STOPPABLE_CLUSTER3, instanceName).getInstanceEnabled(),
        1000);
    ...
    String content = String.format(
        ...
        InstancesAccessor.InstancesProperties.to_be_stopped_instances.name(), "instance14", "instance9", "invalidInstance1");
  ...
  @Test(dependsOnMethods = "testInstanceStoppableWithDisabledAndOfflineInstances")
  ...
  public void testInstanceStoppableZoneBasedWithExceedingMaxOfflineInstances() throws IOException {
    ...
    String content = String.format(
        ...
        "instance2", "instance3", "instance4", "instance5", "invalidInstance",
        InstancesAccessor.InstancesProperties.zone_order.name(), "zone2", "zone1",
        InstancesAccessor.InstancesProperties.to_be_stopped_instances.name(), "invalidInstance1");
    ...
    Set<String> stoppableSet = getStringSet(jsonNode,
        InstancesAccessor.InstancesProperties.instance_stoppable_parallel.name());
    Assert.assertEquals(stoppableSet.size(), STOPPABLE_CLUSTER_MAX_OFFLINE_INSTANCES);
    // If not setting notExceedMaxOfflineInstances=true, instance5 will be included in tbe result.
    Assert.assertTrue(stoppableSet.contains("instance4") && stoppableSet.contains("instance3") && stoppableSet.contains("instance5"));
  ...
  @Test(dependsOnMethods = "testInstanceStoppableZoneBasedWithExceedingMaxOfflineInstances")
  public void testCrossZoneStoppableWithExceedingMaxOfflineInstances() throws IOException {
    System.out.println("Start test :" + TestHelper.getTestMethodName());
    String content = String.format(
        "{\"%s\":\"%s\",\"%s\":[\"%s\",\"%s\",\"%s\",\"%s\",\"%s\", \"%s\"], \"%s\":[\"%s\",\"%s\"], \"%s\":[\"%s\", \"%s\", \"%s\"]}",
        InstancesAccessor.InstancesProperties.selection_base.name(),
    ...
    response = new JerseyUriRequestBuilder(
        "clusters/{}/instances?command=stoppable&skipHealthCheckCategories=CUSTOM_INSTANCE_CHECK,CUSTOM_PARTITION_CHECK"
            + "&notExceedMaxOfflineInstances=true&continueOnFailures=true").format(
        STOPPABLE_CLUSTER2).post(this, Entity.entity(content, MediaType.APPLICATION_JSON_TYPE));
    jsonNode = OBJECT_MAPPER.readTree(response.readEntity(String.class));
  ...
  @Test(dependsOnMethods = "testCrossZoneStoppableWithExceedingMaxOfflineInstances")
