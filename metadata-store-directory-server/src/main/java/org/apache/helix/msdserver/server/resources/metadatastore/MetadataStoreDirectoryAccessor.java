package org.apache.helix.msdserver.server.resources.metadatastore;
...
import org.apache.helix.msdserver.MetadataStoreDirectory;
import org.apache.helix.msdserver.ZkMetadataStoreDirectory;
import org.apache.helix.msdserver.server.common.ContextPropertyKeys;
import org.apache.helix.msdserver.server.common.RestNamespace;
import org.apache.helix.msdserver.server.common.RestUtils;
...
public class MetadataStoreDirectoryAccessor extends AbstractResource {
  ...
  private void postConstruct() {
    RestNamespace restNamespace = getRestNamespace();
    _namespace = restNamespace.getName();

    buildMetadataStoreDirectory(_namespace, restNamespace.getMetadataStoreAddress());
  }
  ...
  private RestNamespace getRestNamespace() {
    RestNamespace RestNamespace = null;
    // A default servlet does not have context property key METADATA, so the namespace
    // is retrieved from property ALL_NAMESPACES.
    if (RestUtils.isDefaultServlet(_servletRequest.getServletPath())) {
      // It is safe to ignore uncheck warnings for this cast.
      @SuppressWarnings("unchecked")
      List<RestNamespace> namespaces = (List<RestNamespace>) _application.getProperties()
          .get(ContextPropertyKeys.ALL_NAMESPACES.name());
      for (RestNamespace ns : namespaces) {
        if (RestNamespace.DEFAULT_NAMESPACE_NAME.equals(ns.getName())) {
          RestNamespace = ns;
          break;
        }
      }
      ...
      RestNamespace =
          (RestNamespace) _application.getProperties().get(ContextPropertyKeys.METADATA.name());
    }
