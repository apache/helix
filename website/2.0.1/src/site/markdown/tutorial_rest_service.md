...
### New in Helix 2.0.0 - Helix Gateway Service

Helix 2.0.0 introduces the **Helix Gateway Service** as the recommended communication mechanism for participant management. The Gateway service uses gRPC for efficient, scalable communication:

* **Improved Performance**: Participants report their shards' current state directly to the gateway
* **Simplified Architecture**: Uses poll-mode channel for efficient state synchronization
* **Better Scalability**: Gateway service handles participant state updates with improved efficiency

For legacy reference, the REST API remains available via helix-rest module.
